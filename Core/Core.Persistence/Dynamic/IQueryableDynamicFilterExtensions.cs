...
public static class IQueryableDynamicFilterExtensions
{
    ...
    public static IQueryable<T> ToDynamic<T>(
        this IQueryable<T> queryable, Dynamic dynamic)
    {
        if (dynamic.Filter is not null) queryable = Filter(queryable, dynamic.Filter);
        if (dynamic.Sort is not null && dynamic.Sort.Any()) queryable = Sort(queryable, dynamic.Sort);
        return queryable;
    }
